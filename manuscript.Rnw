...
\subsection{Individual continuous time state transition model}
...
\subsection{Partitioned survival model}
...
Finally, the complete partitioned survial model is constructed using the survival, utility, and cost mdoels. 
...
<<msm_exact, eval = FALSE, message = FALSE>>=
...
params2_rng <- eval_rng(params2_def, params2)
@
...
rr <- cbind(xbeta(x_rr, params2_rng$rr_12),
            xbeta(x_rr, params2_rng$rr_13))
@
...
icer_tbl(cea_pw_cdtstm, output = "data.table")
@
