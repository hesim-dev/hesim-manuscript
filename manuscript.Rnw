...
The \code{cea_pw()} function computes the ICER. A summary table can be produced with \code{icer()} and \code{format()} can format the table for pretty printing. The argument \code{k} is the WTP threshold and is used to compute the INMB. Estimates of incremental QALYs and incremental costs are computed by averaging over PSA samples. By default, 95\% confidence intervals are presented and are computed using quantiles from the PSA.

<<cea_icer>>=
library("magrittr")
icer(cea_pw_ictstm, k = 100000, 
     strategy_values = strategies$strategy_name[-1]) %>%
  format()
@
