...
\end{tikzpicture}
\caption{Four state disease progression model}\label{fig:four-state-model}
\end{figure}
...
states <- data.table(
  state_id = c(1, 2, 3, 4),
  state_name = c(paste0("State ", 1:3), "Death")
)
...
colnames(tmat) <- rownames(tmat) <- states$state_name
n_trans <- max(tmat)
print(tmat)
@

There are 3 treatment strategies: standard of care (SOC) and two new interventions (New 1 and New 2). For illustration, we will consider a target population approximated by two representative patients of age 60 and 70. The 60-year old patient represents $1/3$ of all patients with the disease while the 70-year old patient represents $2/3$. The \code{hesim_data} class is again used to store this data as well as the number of non-death health states.  
...
The core of a cohort DTSTM is the disease model which is determined by the transition matrices $P_t$. In this example, model parameters related to the transition probabilities for SOC are obtained from a hypothetical analysis of a historical cohort. If data is collected at unevenly spaced intervals, then transitions can be described with transition intensity matrices, $Q_t$, by fitting a continuous time multi-state model designed for panel data (e.g., with the \proglang{R} package \pkg{msm}) \citep{jackson2011multi}. We assume that regression coefficients and a covariance matrix are available from such an analysis and that a piecewise exponential model was fit so that intensities change after year 2. 

<<q_coef, echo = FALSE>>=
trans <- c("1->2", "1->4", "2->1", "2->3", "3->4")
trans_short <- c("12", "14", "21", "23", "34")
var_short <- c("int", "time", "age") 
var_long <- c("intercept", "time[2,Inf)", "age") 
q_coef_int <- log(c(.9, .2, .5, 1.1, 1.5))
q_coef_time <- rep(log(1.2), 5)
q_coef_age <- rep(log(1.003), 5)
q_coef_soc <- data.table(
  trans = rep(trans, 3),
  var = rep(var_long, each = 5),
  name = paste0(rep(var_short, each = 5), "_", rep(trans_short, 3)),
  loghr = c(q_coef_int, q_coef_time, q_coef_age)
)
@
...
<<q_vcov_soc, echo = FALSE>>=
n_vars <- nrow(q_coef_soc)
A <- matrix(runif(n_vars^2) * 2 - 1, ncol = 15)/60
q_vcov_soc <- t(A) %*% A
@
...
We assume that two categories of costs are measured: treatment and medical. The former are the costs of the three treatment strategies and are assumed fixed. The SOC is the cheapest option at \$2,000 per annum while New 1 and New 2 cost \$10,000 and \$12,000 per year, respectively. 
...
To perform a PSA, it is necessary to randomly draw the parameters from suitable probability distributions. These distributions can be defined using \code{define_rng()}. While any random number generation function can be used, a number of commonly used distributions are specifically designed for use within a \code{define_rng()} block (see \code{?rng_distributions}). The built in distributions do not require specification of the number of PSA iterations (since it is defined within the block) and automatically perform certain operations (e.g., sampling from a gamma distribution from means and standard errors by usin methods of moments to derive the underlying shape and scale parameters). We include utility for illustrative purposes but omit the cost parameters since they can be sampled more directly with \code{stateva_tbl()}.
...
The random number generation expressions can are evaluated with \code{eval_rng()}.

<<eval_rng>>=
params_rng <- eval_rng(rng_def, params = params)
@

The transition intensity matrices and relative risks can be combined to derive the relevant transition probability matrices, stored as a \code{tparams_transprobs} object. A \code{tparams_transprobs} is simply a 3-dimensional array of transition probability matrices with a unique predicted matrix for each PSA sample, treatment strategy, representative patient, and (optionally) time interval. As in the prior example, the underlying input data can be constructed with \code{expand}.
...
A complete dataset of ID variables that also incorporate the PSA iterations is constructed with \code{tpmatrix_id()}. The objective is to construct a transition probability for each row in this dataset. The user has complete flexibility to do this.

<<tpmat_id>>=
tpmat_id <- tpmatrix_id(transmod_data, rng_def$n)
head(tpmat_id)
@

In this example we do this by first using the coefficients and input data to to predict transition intensity matrices for SOC (by age group and time interval). This is done by separately predicting intensities for all feasible transitions. There are $n=4$ observations for SOC and $B=1000$ random draws of the coefficients for the PSA. A transition from state $r$ to state $s$ is modeled as $log(q_{rs}) = X\beta^T$ where $X$ is an $n \times 3$ design matrix and $\beta$ is a $B \times 3$ sample of the coefficients. The resulting $n \times B$ matrix can be conveniently flattened into a vector of length $nB$ with the \code{c()} function so that predicted values are stacked by parameter sample as in \code{tpmatrix_id()}. 

<<Q_soc>>=
predict_q_soc <- function(data, beta){
...
q_soc <- predict_q_soc(transmod_data, params_rng$q_coef_soc)
head(q_soc)
@

The predicted transition intensities are, in turn, use to form the complete transition intensity matrix. Specifically, \code{qmatrix()} forms an array of transition intensity matrices where each row sums to $0$ so that $q_{rr} = \sum_{s \neq r} -q_{rs}$.

<<Q_soc>>=
qmat_soc <- qmatrix(q_soc, tmat)
qmat_soc[,, 1]
@

Transition probability matrices are constructed from the transition intensity matrices using the matrix exponential \citep{jackson2011multi}. In continuous time, $P(t) = \exp(tQ)$ so a transition probability matrix with model cycles of length $t$ years is given by $\exp(Q_t)$. In this example we will assume model cycles are 1/2 of a year. 

<<P_soc>>=
pmat_soc <- expmat(qmat_soc, t = 1/2)
pmat_soc[,, 1]
@

Transition probability matrices can be derived for treatment strategies New 1 and New 2 using the relative risks. Relative risks are predicted for each row in the \code{tmat_id} dataset above.

<<rr>>=
x_rr <- as.matrix(transmod_data[, .(soc, new1, new2)])
rr_12 <- c(x_rr %*% t(params_rng$rr_12))
rr_14 <- c(x_rr %*% t(params_rng$rr_14))
rr <- cbind(rr_12, rr_14)
@

Given reference transition probability matrices (i.e., those for SOC), we can apply the relative risks for the other treatment strategies using the \code{apply_rr()} function. An array of transition probability matrices is returned with one matrix for each row in \code{rr}.
...
Finally, the array of transition probability matrices is combined with the relevant ID variables indexing each array slice to create the \code{tparams_transprobs} object. 

<<apply_rr>>=
tprobs <- tparams_transprobs(pmat, tpmat_id)
@

The utility and cost parameters are much more straightforward to store. As in the previous example we consider two cost categories and store parameter values in a \code{stateval_tbl}.

<<tx_costs>>=
txcost_tbl <- stateval_tbl(
  data.table(
    strategy_id = 1:3,
...
The simulation proceeds the same way as in the prior examples. We first create the transition, cost, and utility models, here specifying that model cycles are 6 months long since the model is in discrete time.  

<<transmod>>=
transmod <- CohortDtstmTrans$new(params = tprobs, cycle_length = 1/2)
@
...
# Costs
txcostmod <- create_StateVals(txcost_tbl, n = rng_def$n)
medcostmod <- create_StateVals(medcost_tbl, n = rng_def$n)
costmods <- list(Treatment = txcostmod,
                 Medical = medcostmod)
@
...
<<sim_stateprobs>>=
econmod$sim_stateprobs(n_cycles = 45 * 2)
@
...
cea_pw_out <- cea_pw(ce_sim_grp, comparator = 1, dr_costs = .03, dr_qalys = .03)
icer_tbl(cea_pw_out, output = "data.table")
@

They can also be aggregated across groups using the weighted specified in the \code{hesim_data} object above. 

<<icer>>=
ce_sim <- econmod$summarize()
cea_pw_out <- cea_pw(ce_sim, comparator = 1, dr_costs = .03, dr_qalys = .03)
icer_tbl(cea_pw_out, output = "data.table")
@
