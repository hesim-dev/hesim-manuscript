...
\node[draw,fill=white, minimum height=.8cm, minimum width=2.5cm, font=\footnotesize] at (5, 0){CEA};
...
<<ictstm_drugcost>>=
drugcost_tbl <- stateval_tbl(drugcost_dt, dist = "fixed")
drugcost_tbl
@
...
library("ggplot2")
autoplot(ictstm$stateprobs_, labels = labs_indiv, ci = FALSE) 
@
...
fit_pfs_soc_wei <- flexsurvreg(Surv(pfs_time, pfs_status) ~ female,
  data = onc_pfs_os_soc, dist = "weibull")
fit_os_soc_wei <- flexsurvreg(Surv(os_time, os_status) ~  female,
  data = onc_pfs_os_soc, dist = "weibull")
...
# Fit models
fit_pfs_wei <- flexsurvreg(Surv(pfs_time, pfs_status) ~ female + new1 + new2,
  data = onc_pfs_os, dist = "weibull")

fit_os_wei <- flexsurvreg(Surv(os_time, os_status) ~ female + new1 + new2,
  data = onc_pfs_os, dist = "weibull")
...
survmods <- create_PsmCurves(survmods_params, input_data = survmods_data)
@
...
<<psm_econmod>>=
psm <- Psm$new(survival_models = survmods, utility_model = utilitymod,
...
for (i in 1:length(wei_fits)){
  exp_fits[[i]] <- flexsurvreg(Surv(time, status) ~ 1, data = onc3, 
    subset = (transition_id == i & strategy_name == "SOC"),
    dist = "exponential")
}
...
Identifiers for the complete array of transition matrices (inclusive of SOC and the new interventions) are generated with \code{tpmatrix_id()}.
...
<<apply_rr>>=
pmat <- apply_rr(pmat_soc, rr = rr, index = list(c(1, 2), c(1, 3)))
...
cea_pw_ictstm <- cea_pw(ce_sim_ictstm, comparator = 1, dr_qalys = .03, 
  dr_costs = .03, k = wtp)
@
