...
If many studies are available, then relative treatment effects can be estimated by means of a network meta-analysis. We assume that an NMA was performed to estimate the relative risk of transitions from state 1 to both state 2 and death. Treatment strategy 1 was the reference arm. Estimation is performed in terms of the log relative risk since it is approximately normally distributed. 

The utility associated with each state is assumed to follow a beta distribution, which is bounded between 0 and 1. The beta distribution is defined in terms of 2 shape parameters, but can be derived from the mean and standard error using the method of moments. Point estimates for utility in states 1, 2, and 3 are 0.9, 0.75, and 0.5, respectively. Standard errors in the same three states are 0.02, 0.03, and 0.05. Utility is always assumed to be 0 in death states in \pkg{hesim}.
