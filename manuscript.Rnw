...
<<labels_indiv>>=
labs_indiv <- get_labels(hesim_dat, grp_label = NULL)
@
...
A number of common measures can be used to represent decision uncertainty from a CEA, including cost-effectiveness planes, cost-effectiveness acceptability curves (CEAC), cost-effectiveness acceptability frontiers (CEAF), and the expected value of perfect information (EVPI). Plots can be quickly generated with \code{plot_ceplane()}, \code{plot_ceac()}, \code{plot_ceaf()}, and \code{plot_evpi()} functions, respectively. Example plots from the code that follows are shown in Figure~\ref{fig:cea-plots}
