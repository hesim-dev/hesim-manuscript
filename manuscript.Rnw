...
Model parameters related to transition probabilities for SOC are obtained from a hypothetical analysis of a historical cohort. If data is collected at unevenly spaced intervals, then transitions can be described with transition intensity matrices, $Q$, by fitting a continuous time multi-state model designed for panel data (e.g., with the \proglang{R} package \pkg{msm}) \citep{jackson2011multi}. We assume that regression coefficients and a covariance matrix for $Q$ are available from such an analysis.

<<q_coef, echo = FALSE>>=
trans <- c("1->2", "1->4", "2->1", "2->3", "3->4")
q_coef_int <- log(c(.9, .2, .5, 1.1, 1.5))
q_coef_time <- rep(log(1.2), 5)
q_coef_age <- rep(log(1.05), 5)
q_coef_soc <- data.table(
  trans = rep(trans, 3),
  var = rep(c("intercept", "time[2,Inf)", "age"), each = 5),
...
A <- matrix(runif(n_vars^2) * 2 - 1, ncol = 15)/30
q_vcov_soc <- t(A) %*% A
@
...
To perform a PSA, it is necessary to randomly draw the parameters from suitable probability distributions. These distributions can be defined using \code{define_rng()}. While any random number generation function can be used, a number of commonly used distributions are specifically designed for use within a \code{define_rng()} block (see \code{?rng_distributions}). The random number generation expressions can either be directly evaluated with \code{eval_rng()} or passed to \code{define_model()} and evaluated during construction of the economic model. We will take the latter approach here.

<<define_rng>>=
rng_def <- define_rng({
  list( # Parameters to return
    q_coef = multi_normal_rng(q_coef_soc$loghr, q_vcov_soc),
    lrr = lognormal_rng(lrr_est, lrr_se),
...
<<eval_rng, eval = FALSE, echo = FALSE>>=
eval_rng(rng_def, params = params)
@
